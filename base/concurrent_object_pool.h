...
#ifndef IROS_BASE_CONCURRENT_OBJECT_POOL_H_
#define IROS_BASE_CONCURRENT_OBJECT_POOL_H_
...
bool CCObjectPool<T>::FindFreeHead(Head* head)
{
    ...
    do {
        if (iros_unlikely(old_head.node == nullptr)) { return false; }
        new_head.node  = old_head.node->next;
        new_head.count = old_head.count + 1;
    } while (!free_head_.compare_exchange_weak(
        old_head, new_head, std::memory_order_acq_rel, std::memory_order_acquire));
    *head = old_head;
    return true;
}
...
{
    Head free_head;
    if (iros_unlikely(!FindFreeHead(&free_head))) { return nullptr; }
...
#endif   // IROS_BASE_CONCURRENT_OBJECT_POOL_H_
